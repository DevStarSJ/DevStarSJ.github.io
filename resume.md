...
- **배경**: 해당 엔진은 설계 당시 현재 수준의 부하를 전제하지 않아, 트래픽이 늘면서 안정성 이슈가 반복되는 상태였습니다. 구조를 직접 개선하기 어려운 레거시였기 때문에 운영 부담이 지속적으로 누적되고 있었습니다.
...
**AI 코드리뷰 자동화 도입 및 구조 개선**

- 모든 Pull Request에 AI 기반 자동 코드리뷰(Codex)가 수행되도록, **AWS Lambda와 GitHub organization webhook을 연동한 리뷰 봇을 구축**했습니다. 조직 내 모든 저장소의 PR에 리뷰가 자동 요청되는 구조였습니다.
- 파일럿 운영 결과 리뷰 품질은 유의미하다고 판단했으나, **전사 리뷰 요청이 단일 계정의 토큰 quota에 집중되는 구조적 한계**를 확인했습니다.
- 이에 중앙 webhook을 제거하고, **저장소별 담당자가 각자 계정에서 자동 리뷰를 활성화하는 분산 구조로 전환**했습니다. 비용이 개인별 quota로 분산되는 동시에, 중앙 리뷰 봇 인프라의 운영 부담도 함께 제거되었습니다.
- 전환에 앞서 도구의 동작 방식(저장소 단위 설정, 브랜치 필터 부재로 인한 quota 소모 특성)을 사전 검증했고, 저장소별 담당자 매핑을 문서화하여 사내에 공지했습니다.
