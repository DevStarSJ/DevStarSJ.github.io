...
하지만 를 EKS에 띄우기 위해서는 **Ingress Controller**가 필수적으로 필요하다. AWS에서는 이것을 **AWS LoadBalancer Controller**라고 이름을 정했다. 이것에 대한 생성을 우리가 yaml파일을 보고 직접이해하기는 힘들다. 500 line이 넘는 yaml파일을 제공해주는데 그냥 그것을 사용해야한다.
