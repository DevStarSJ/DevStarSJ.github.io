...
먼저 서버, 웹페이지의 1 story point에 대해서 각각 정의하였다. 그런 다음 항목도 적고 쉬워보이는 **사용자 관리**에 대해서는 내가 리딩하면서 진행했다. 그러면서 페이지 구현에 필요한 UI Component를 포스트잇에 적어서 벽에 Component라고 적어둔 이젤패드에다가 붙였으며, 각 항목별로 서버, 웹페이지 개발이라는 Story를 Jira에 생성하였으며, 다 같이 Story Point에 대해서 정하였다. 이렇게 대메뉴 하나에 대해서 진행하는 과정을 모두와 공유한 뒤 나머지 대메뉴 7개 중에 하나는 가장 마지막에 해야할 뿐더러 신규 기능이라서 판단을 지금 하는게 의미없다고 판단해서 제외시켰다. 개발팀 인원으로 2인 1조로 해서 총 3개조로 편성하였고, 남은 6개 중에 항목이 많은 3개를 3개조에 분배하였다. 작업을 하다가 모르는 항목이 나오면 해당 항목을 작성한 이해관계자를 찾아가서 이야기를 듣고 story point를 산출해 달라고 했다.